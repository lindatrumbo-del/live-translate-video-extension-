...
export class SubtitlesProcessor {
  ...
  static async fetchSubtitles(subtitlesObject) {
    const { source, isAutoGenerated, format } = subtitlesObject;
    let { url } = subtitlesObject;
    if (source === "youtube") {
      // https://github.com/ilyhalight/voice-over-translation/issues/1303
      const poToken = YoutubeHelper.getPoToken();
      if (poToken) {
        const deviceParams = YoutubeHelper.getDeviceParams();
        url += `&potc=1&pot=${poToken}&${deviceParams}`;
      }
    }
